...
class OrderResponse(OrderCreate):
    ...
    def total_price(self) -> int:
        """ Вычисляет общую стоимость всех продуктов в заказе """
        return sum(pio.price * pio.quantity for pio in self.products)


    @computed_field
    @property
    def total_quantity(self) -> int:
        """ Вычисляет общее количество всех продуктов в заказе """
        return sum(pio.quantity for pio in self.products) 
